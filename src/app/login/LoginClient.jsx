...
export default function LoginClient() {
  ...
  const [isTokenLogging, setIsTokenLogging] = useState(false)
  const tokenInputRef = useRef(null)
  const { login, isLoggedIn, checkAuthStatus } = useAuth()
  ...
  // Auto-login if ?token= is in the URL
  useEffect(() => {
  ...
  return (
    <div style={styles.container}>
      <div style={styles.card}>
        ...
        <button
          onClick={() => { setShowTokenModal(true); setTokenError('') }}
      ...
      {showTokenModal && (
        <div style={styles.modalOverlay} onClick={() => setShowTokenModal(false)}>
          <div style={styles.modal} onClick={e => e.stopPropagation()}>
            <div style={styles.modalHeader}>
              <span style={styles.modalTitle}>Enter Access Code</span>
              <button onClick={() => setShowTokenModal(false)} style={styles.modalClose}>✕</button>
            </div>
            ...
            <input
              ...
              onKeyDown={e => e.key === 'Enter' && !isTokenLogging && tokenInput.trim() && handleTokenLogin(tokenInput)}
            ...
            />
            {tokenError && (
              <div style={styles.tokenError}>{tokenError}</div>
            )}
            <button
              onClick={() => handleTokenLogin(tokenInput)}
              disabled={isTokenLogging || !tokenInput.trim()}
              style={{
                ...styles.tokenSubmit,
                ...(isTokenLogging || !tokenInput.trim() ? styles.signInButtonDisabled : {})
              }}
            >
              {isTokenLogging ? 'Signing in...' : 'Continue'}
            </button>
          </div>
        </div>
      )}
